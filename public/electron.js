...
async function startWhisperServer() {
  try {
    const appPath = app.getAppPath();
    const whisperScriptPath = path.join(appPath, 'whisper_server.py');
    ...
    if (!scriptPath) {
      console.warn('Whisper server script not found. Transcription will fallback to OpenAI API.');
      console.warn('Expected paths:', possiblePaths);
      return;
    }
    
    console.log('Starting Whisper server from:', scriptPath);
    ...
    try {
      ...
      try {
        ...
        console.warn('Python not found. Whisper server cannot start. Please install Python 3.');
        return;
      }
    }
    
    // Start the server
    whisperServerProcess = spawn(pythonCommand, [scriptPath], {
      cwd: path.dirname(scriptPath),
      env: { ...process.env, WHISPER_PORT: String(WHISPER_PORT) },
    ...
    whisperServerProcess.stderr.on('data', (data) => {
      console.error(`[Whisper Server Error] ${data.toString().trim()}`);
    });
    
    whisperServerProcess.on('error', (error) => {
    ...
    try {
      ...
      const checkRequest = http.get(`http://localhost:${WHISPER_PORT}/health`, (res) => {
        if (res.statusCode === 200) {
          console.log('Whisper server is running and healthy');
        }
      });
      checkRequest.on('error', () => {
        console.warn('Whisper server health check failed, but continuing...');
      });
      checkRequest.setTimeout(1000, () => checkRequest.destroy());
    } catch (e) {
      console.warn('Could not verify Whisper server health:', e.message);
    }
    
  } catch (error) {
    console.error('Error starting Whisper server:', error);
    whisperServerProcess = null;
  }
}
...
function stopWhisperServer() {
  if (whisperServerProcess) {
    ...
    try {
      ...
      console.error('Error stopping Whisper server:', error);
    }
  }
}
