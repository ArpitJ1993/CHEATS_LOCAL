...
import MeetingsTranscription from './MeetingsTranscription';
...
const MeetingsHeader = styled.div`
  ...
  align-items: center;
  -webkit-app-region: drag; /* Make meetings header draggable */
  margin-bottom: 16px;
`;

const MeetingsHeaderControls = styled.div`
...
export const MeetingsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (window as any).__testTranscription = () => {
      const event = new CustomEvent('local-transcription', {
        detail: { source: 'microphone', text: 'Test transcription from window' }
      });
    ...
    return () => {
      ...
      delete (window as any).__testTranscription;
    };
  ...
  useEffect(() => {
    ...
    // Remove any existing script first to ensure clean re-initialization
    const existingScript = document.querySelector('script[src*="meetingsRenderer.js"]');
    if (existingScript) {
      existingScript.remove();
    }
    
    const script = document.createElement('script');
    // Add cache-busting query parameter to ensure script re-executes
    const cacheBuster = `?v=${Date.now()}`;
    script.src = (isDev ? '/meetingsRenderer.js' : 'meetingsRenderer.js') + cacheBuster;
    script.type = 'module';
    script.async = true;
    
    // Wait for script to load, then re-initialize
    script.onload = () => {
      // Small delay to ensure the script has executed
      setTimeout(() => {
        // Re-initialize to ensure event listeners are attached
        const reinitFn = (window as any).__reinitializeMeetingsRenderer;
        if (typeof reinitFn === 'function') {
          reinitFn().catch((err: any) => {
            console.error('Failed to re-initialize meetings renderer:', err);
          });
        } else {
          // Fallback: refresh microphone options if re-init not available
          const refreshFn = (window as any).__refreshMicrophoneOptions;
          if (typeof refreshFn === 'function') {
            refreshFn().catch((err: any) => {
              console.error('Failed to refresh microphone options:', err);
            });
          }
        }
      }, 100);
    ...
    // Also try to re-initialize immediately if script is already loaded (cached)
    setTimeout(() => {
      const reinitFn = (window as any).__reinitializeMeetingsRenderer;
      if (typeof reinitFn === 'function') {
        reinitFn().catch((err: any) => {
          // Script might not be loaded yet, that's okay
        });
      }
    }, 200);
    ...
    return () => {
      // Cleanup: Stop all audio captures when leaving the meetings page
      const stopAllFn = (window as any).__stopAllCaptures;
      if (typeof stopAllFn === 'function') {
        stopAllFn().catch((err: any) => {
          console.error('Failed to stop captures on page leave:', err);
        });
      }
